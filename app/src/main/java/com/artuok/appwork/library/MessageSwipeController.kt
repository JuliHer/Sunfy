...
class MessageSwipeController(private val context: Context, private val element : ArrayList<Item>, private val blockAll : Boolean, private val swipeControllerActions: SwipeControllerActions) :
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        if (startTracking) {
            if (!isVibrate && mView.translationX >= convertTodp(150)) {
        ...
        val x: Int = if (mView.translationX > convertTodp(200)) {
            convertTodp(200) / 2
        } else {
        ...
        shareRound.colorFilter =
            PorterDuffColorFilter(ContextCompat.getColor(context, R.color.black_transparent_500), PorterDuff.Mode.MULTIPLY)
